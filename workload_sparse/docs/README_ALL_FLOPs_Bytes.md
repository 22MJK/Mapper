...
| ncu 指标                  | 含义                   |
|:----------------------- |:-------------------- |
| `dram__bytes_read.sum`  | GPU 内核从 DRAM 读取的总字节数 |
| `dram__bytes_write.sum` | GPU 内核向 DRAM 写入的总字节数 |
| `dram__bytes.sum`       | 总 DRAM 流量 = 读 + 写    |

### 拟合流程

1. 选 7+ 组不同规模 $N$（如 256, 512, 1024, 2048, 4096, 6144, 8192）
2. 每组用 ncu 采集实际 DRAM Bytes
3. 计算 $k_N = \text{Bytes}_{\text{ncu}} \;/\; \text{Bytes}_{\text{theory}}$
4. 用 MAPE 最小化拟合最优 $k$：$\;k_{\text{opt}} = \arg\min_k \sum \left|\frac{k \cdot \text{theory}_N - \text{ncu}_N}{\text{ncu}_N}\right|$

得到的系数填入本文表的 `BytesGPU拟合` 列。例如 TRSV 的 0.50× 表示 GPU 实际 DRAM 仅为理论值的 50%，因为 L2 Cache 截获了大量重复访问。

---

## GPU FLOPs 拟合

Ada Lovelace (SM 8.9) 不支持旧版 SASS 指令计数器。

---

## CPU 拟合

在Intel VTune 上进行，但只读取到了系统运行的FLOPs和Bytes，找不到精确到算子的。
